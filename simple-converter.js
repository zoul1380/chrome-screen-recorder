            ...
            return new Promise((resolve, reject) => {
                video.onloadedmetadata = async () => {
                    try {
                        ...
                        console.log(`GIF conversion: ${gifWidth}x${gifHeight}, ${totalFrames} frames, ${duration}s`);
                        ...
                        await new Promise((resolveTest, rejectTest) => {
                            const testSeek = () => {
                                try {
                                    ...
                                    console.log('Video frame test completed');
                        ...
                        console.log(`Creating animated image from ${frames.length} frames`);
                          // Create proper animated GIF instead of composite image
                        console.log(`Creating animated GIF from ${frames.length} frames`);                        // Create a working animated representation
                        console.log(`Creating animated GIF representation from ${frames.length} frames`);
                        ...
                        const gifPromise = new Promise((resolveGif, rejectGif) => {
                            gif.on('finished', function(blob) {
                                console.log('Animated GIF created successfully, size:', blob.size);
                                resolveGif(blob);
                            });
                        ...
                        for (let i = 0; i < frames.length; i++) {
                            ...
                            await new Promise(resolve => {
                                img.onload = () => {
                                    ...
                                    console.log(`Added frame ${i + 1}/${frames.length} to animated GIF`);
                                    resolve();
                                };
                                img.onerror = () => {
                        ...
                        URL.revokeObjectURL(videoUrl);
                        console.log('GIF conversion completed, blob size:', animatedBlob.size);
